...
def get_personal_horoscope():
    profile = "ชาย เกิด วันอังคาร 26 / 05 / 2541 ปีขาล (ราศีพฤษภ ♉️)"
    ...
    prompt = (
        ...
        "และส่วนท้ายสุดให้ระบุ 👕 สีเสื้อเสริมดวง (แนะนำมา 3-5 สี พร้อมระบุว่าแต่ละสีเสริมด้านใด เช่น การงาน, การเงิน, ความรัก), 🚫 สีกาลกิณี(ห้ามใส่) และ 🔢 เลขมงคลประจำวัน "
        "ขอให้เนื้อหากระชับ อ่านง่าย ให้กำลังใจ ใช้ภาษาเป็นกันเอง และมีความยาวรวมไม่เกิน 250 คำ"
    )
...
def get_lotto_prediction(lotto_type):
    ...
    # ข้อมูลสถิติย้อนหลังจำลอง (เปลี่ยนเป็นของจริงได้ในอนาคตด้วย API/Scraping)
    if "ไทย" in lotto_type:
        history_stats = "งวดก่อนหน้า: [432, 11], [098, 56], [754, 33]"
    else:
        history_stats = "งวดก่อนหน้า: [85, 23], [14, 90], [67, 45]"

    prompt = (
        f"ผู้ใช้โปรไฟล์ {profile} ต้องการขอแนวทางเลขเด็ดสำหรับ '{lotto_type}' ประจำ{today_date} "
        "โดยให้ขึ้นต้นประโยคด้วยการระบุวันที่วันนี้ให้ชัดเจน และแนะนำผู้ใช้เพิ่มเติมว่า 'สำหรับวันนี้มีหวยอะไรออกบ้าง' (พิจารณาจากวันและวันที่ปัจจุบัน เช่น หวยฮานอยออกทุกวัน, หวยลาวพัฒนาออกจันทร์-ศุกร์, หวยรัฐบาลไทยออก 1 และ 16) "
        f"จากนั้นให้วิเคราะห์สถิติจากผลหวยย้อนหลังดังนี้: {history_stats} "
        "ให้คุณคำนวณ 'ความน่าจะเป็นทางคณิตศาสตร์' จากสถิตินี้ควบคู่กับโหราศาสตร์ "
        "และฟันธง 'เลขเด่นที่มีโอกาสออกสูงสุด' (ระบุเลขท้าย 2 ตัว และ 3 ตัว) พร้อมอธิบายเหตุผลสั้นๆ จากสถิติ "
        "พร้อมปิดท้ายด้วยข้อความอวยพรให้โชคดีถูกรางวัล"
    )
    answer = call_openai_api(prompt)
...
def send_telegram_message(chat_id=None, text=None, reply_markup=None, use_accept_buttons=False):
    ...
    try:
        ...
        logger.error(f"❌ ไม่สามารถส่งข้อความได้: {e}")
...
def setup_bot():
    ...
    if RENDER_EXTERNAL_URL:
        ...
        logger.info(f"🔗 ตั้งค่า Webhook ไปที่: {webhook_url}")
...
def webhook():
    ...
    if update and "callback_query" in update:
        ...
        if str(chat_id) != str(CHAT_ID):
            ...
            requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/answerCallbackQuery", json={
                ...
                "text": "⛔ ขออภัย คุณไม่ได้รับอนุญาตให้ใช้งานบอทนี้ครับ",
                "show_alert": True
            })
            return "OK", 200
            
        logger.info(f"Callback received from {chat_id}: {data}")
    ...
    if update and "message" in update:
        ...
        # 🔒 ระบบล็อก (Lock System): ตรวจสอบสิทธิ์การใช้งาน
        if str(chat_id) != str(CHAT_ID):
            logger.warning(f"Unauthorized message from chat_id: {chat_id} - Text: {text}")
            send_telegram_message(chat_id, "⛔ ขออภัย บอทนี้เป็นบอทดูดวงส่วนตัว คุณไม่ได้รับอนุญาตให้ใช้งานครับ")
            return "OK", 200
...
# ปิดการทำงาน Scheduler ส่งดวงตอนเช้า
# tz = pytz.timezone('Asia/Bangkok')
